...
class Request
{
    ...
    public static function custom($method, $target, $header,  $content, $authbool=true, $sessiondelete = false)
    {
        ...
        if (isset(self::$confData['PL']['urlExtern']) && isset(self::$confData['PL']['url'])){
            if (strpos($target,self::$confData['PL']['urlExtern'].'/')===0){
                // es wurde eine globale URL erkannt, welche zu einer lokalen umgewandelt werden kann
                $target = self::$confData['PL']['url'].substr($target,strlen(self::$confData['PL']['urlExtern']));
            }
        }

        $done = false;
        if (!CConfig::$onload && isset(self::$confData['PL']['url']) && strpos($target,self::$confData['PL']['url'].'/')===0 && file_exists(dirname(__FILE__) . '/request_cconfig.json')){
