...
class PlattformDatenbanknutzer
{
    ...
    public static function install($data, &$fail, &$errno, &$error)
    {
        ...
        if (!$fail && ((isset($data['action']) && $data['action']=='update') ||isset($data['DB']['db_user_override_operator']) && $data['DB']['db_user_override_operator'] === 'override')){
            ...
            Installation::log(array('text'=>'sql = '.$sql2));
        ...
        $userExists = false;
        if (!$fail){
            Installation::log(array('text'=>'prüfe ob der Nutzer existiert ('.$data['DB']['db_user_operator'].')'));
            ...
            if ($result["errno"] !== 0 || !isset($result["content"])){
                $fail = true; $errno = $result["errno"];$error = isset($result["error"]) ? $result["error"] : '';
                Installation::log(array('text'=>'Fehler: '.json_encode($result),'logLevel'=>LogLevel::ERROR));
            } else {
                $result = DBJson::getRows($result['content']);
                if (count($result)>0 && isset($result[0]['exists']) && $result[0]['exists']>0) {
                    Installation::log(array('text'=>'Der Nutzer existiert'));
                    $userExists = true;
                } else {
                    Installation::log(array('text'=>'Der Nutzer existiert nicht'));
                }
            }
            $data['DB']['db_name'] = $oldName;
        }
        ...
        if (!$fail && !$userExists){
            Installation::log(array('text'=>'Erzeuge einen neuen Nutzer ('.$data['DB']['db_user_operator'].')'));
            ...
            $logSql = "CREATE USER '{$data['DB']['db_user_operator']}'@'%'".
                    "IDENTIFIED BY *****;";
            $logSql.= "GRANT CREATE VIEW,EXECUTE,ALTER ROUTINE,CREATE ROUTINE,SHOW VIEW,CREATE TEMPORARY TABLES,INDEX,ALTER,SELECT,INSERT,UPDATE,DELETE,CREATE,DROP,TRIGGER ".
                    "ON `{$oldName}`.* ".
                    "TO '{$data['DB']['db_user_operator']}'@'%'; ";
            ...
            $logSql.= "GRANT CREATE VIEW,EXECUTE,ALTER ROUTINE,CREATE ROUTINE,SHOW VIEW,CREATE TEMPORARY TABLES,INDEX,ALTER,SELECT,INSERT,UPDATE,DELETE,CREATE,DROP,TRIGGER ".
                    "ON `{$oldName}`.* ".
                    "TO '{$data['DB']['db_user_operator']}'@'localhost';";
            Installation::log(array('text'=>'sql = '.$logSql));
            $result = DBRequest::request2($sql, false, $data);
