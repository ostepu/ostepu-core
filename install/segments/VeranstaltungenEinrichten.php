        ...
        if (!$fail){
            ...
            if (isset($result[0]['content']) && isset($result[0]['status']) && $result[0]['status'] === 200){
                ...
                foreach($courses as $course){
                    Installation::log(array('text'=>'erstelle Anfrage: POST '.$data['PL']['url'].'/logic/LCourse/course'.' Content: '.Course::encodeCourse($course)));
                ...
                foreach($courses as $course){
                    ...
                    if (isset($result['content']) && isset($result['status']) && $result['status'] === 201){
                        ...
                        Installation::log(array('text'=>'Erfolgreich ('.$course->getName().')'));
                    } else {
                        ...
                        Installation::log(array('text'=>'Fehler: status = '.$res[$course->getId()]['status'], 'logLevel'=>LogLevel::ERROR));
                    }
                    $i++;
