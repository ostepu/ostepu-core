				...
				foreach($results as $res){
					...
					foreach ($definitions as $definition){
						if ($definition->getId() === $res->getId()){
							...
							$result3 = Request::get($definition->getAddress().'/info/links',array(),'');
							if (isset($result3['content']) && isset($result3['status']) && $result3['status'] === 200){
								$calls = json_decode($result3['content'], true);
