...
class DBTransaction
{
   ...
   public function addExerciseTransaction( $callName, $input, $params = array())
    ...
    {
        return $this->_component->callSqlTemplate('cleanTransactions',dirname(__FILE__).'/Sql/CleanTransactions.sql',$params,201,'Model::isCreated',array(new Course()),'Model::isProblem',array(new Course()),false);
    }
    ...
    {   
        ...
        $profiles['readonly']->addRule(GateRule::createGateRule(null,'httpCall',$myName,'GET /transaction/:path+',null));
        
        $profiles['general'] = GateProfile::createGateProfile(null,'general');
        $profiles['general']->setRules($profiles['readonly']->getRules());
        $profiles['general']->addRule(GateRule::createGateRule(null,'httpCall',$myName,'DELETE /transaction/:path+',null));
        $profiles['general']->addRule(GateRule::createGateRule(null,'httpCall',$myName,'PUT /transaction/:path+',null));
        $profiles['general']->addRule(GateRule::createGateRule(null,'httpCall',$myName,'POST /transaction/:path+',null));
