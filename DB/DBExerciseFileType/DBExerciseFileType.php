...
class DBExerciseFileType
{
    ...
    public function __construct($conf)
    {
        ...
        $this->_app->put('/' . $this->getPrefix() . '(/exercisefiletype)/:eftid(/)',
                        array($this,'editExerciseFileType'));
        
        // DELETE DeleteExerciseFileType
        $this->_app->delete('/' . $this->getPrefix() . '(/exercisefiletype)/:eftid(/)',
                           array($this,'deleteExerciseFileType'));
        
        // POST AddExerciseFileType
        $this->_app->post('/' . $this->getPrefix() . '(/)',
                         array($this,'addExerciseFileType'));  
        
        // GET GetExerciseFileType
        $this->_app->get('/' . $this->getPrefix() . '(/exercisefiletype)/:eftid(/)',
                        array($this,'getExerciseFileType'));
                        
        // GET GetExerciseExerciseFileTypes
        $this->_app->get('/' . $this->getPrefix() . '/exercise/:eid(/)',
                        array($this,'getExerciseExerciseFileTypes'));
                        
        // GET GetAllExerciseFileTypes
        $this->_app->get('/' . $this->getPrefix() . '(/exercisefiletype)(/)',
                        array($this,'getAllExerciseFileTypes'));
    ...
    {
        Logger::Log("starts PUT EditExerciseFileType",LogLevel::DEBUG);
        ...
        foreach ($insert as $in){
            ...
            if ($result['status']>=200 && $result['status']<=299){
                ...
                Logger::Log("PUT EditExerciseFileType failed",LogLevel::ERROR);
    ...
    {
        Logger::Log("starts DELETE DeleteExerciseFileType",LogLevel::DEBUG);
        ...
        if ($result['status']>=200 && $result['status']<=299){
            ...
            Logger::Log("DELETE DeleteExerciseFileType failed",LogLevel::ERROR);
            $this->_app->response->setStatus(isset($result['status']) ? $result['status'] : 409);
            $this->_app->stop();
        }
    }
    ...
    {
        Logger::Log("starts POST SetExerciseFileType",LogLevel::DEBUG);
        ...
        foreach ($insert as $in){
            ...
            if ($result['status']>=200 && $result['status']<=299){
                ...
                Logger::Log("POST SetExerciseFileType failed",LogLevel::ERROR);
                $this->_app->response->setStatus(isset($result['status']) ? $result['status'] : 409);
    ...
    public function get($functionName,$sqlFile,$userid,$courseid,$esid,$eid,$etid,$eftid,$singleResult=false)
    {
        ...
        if ($result['status']>=200 && $result['status']<=299){ 
            ...
            if ($query->getNumRows()>0){
                $res = ExerciseFileType::ExtractExerciseFileType($query->getResponse(),$singleResult); 
                $this->_app->response->setBody(ExerciseFileType::encodeExerciseFileType($res));
    ...
    {      
        $this->get("GetAllExerciseFileTypes",
    ...
    {      
        $this->get("GetExerciseExerciseFileTypes",
